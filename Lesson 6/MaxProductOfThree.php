 ...
 * Your goal is to find the maximal product of any triplet. Write a function: function solution($A);
...
function solution($A) {
    $length = count($A);
    sort($A);
    $start_product = $A[0] * $A[1] * $A[$length - 1];
    $finish_product = $A[$length - 1] * $A[$length - 2] * $A[$length - 3];
    return $start_product > $finish_product ? $start_product : $finish_product;
}
